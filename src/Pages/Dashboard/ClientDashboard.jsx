...
const ClientDashboard = ({ user }) => {
  ...
  const loadAccount = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Hisob ma'lumotlarini olishda xato.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Box className="h-full overflow-auto pr-1">
      <Box className="mb-5">
        ...
        <Typography variant="body2" className="mt-1 text-slate-500">
          Salom, {user?.first_name || "Mijoz"}. Xaridlaringiz, to'lovlaringiz va
          qolgan qarzingiz shu yerda ko'rinadi.
        </Typography>
      </Box>
      ...
      <Paper elevation={0} className="mb-5 overflow-hidden border border-slate-200" sx={{ borderRadius: 2 }}>
          ...
          <Table size="small" sx={{ minWidth: 760 }}>
            ...
            <TableBody>
              {account.client_sales?.length ? (
                ...
                <TableRow><TableCell colSpan={7} align="center">Hali xarid yozuvi yo'q.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </Box>
      ...
      <Paper elevation={0} className="overflow-hidden border border-slate-200" sx={{ borderRadius: 2 }}>
        <Box className="border-b border-slate-200 px-5 py-4">
              ...
              {account.client_payments?.length ? (
                account.client_payments.map((payment) => (
                  <TableRow key={payment.id} hover>
                    <TableCell><Typography fontWeight={800}>{money(payment.amount)}</Typography></TableCell>
                    <TableCell>{payment.product_name || (payment.client_sale_id ? `Savdo #${payment.client_sale_id}` : "Umumiy to'lov")}</TableCell>
                    <TableCell>{payment.note || "-"}</TableCell>
                    <TableCell>{date(payment.paid_at)}</TableCell>
                  </TableRow>
                ))
              ) : (
                <TableRow><TableCell colSpan={4} align="center">Hali alohida to'lov yozuvi yo'q.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </Box>
