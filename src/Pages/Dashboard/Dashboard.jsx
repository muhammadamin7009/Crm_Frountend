...
const WorkerDashboard = ({ user }) => {
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    try {
      ...
      const [monthRes, todayRes, departmentsRes, balanceRes] =
        await Promise.all([
          getWorkerOutputs({
            ...monthRange,
            offset: 0,
      ...
      setMonthTotals(
        monthRes.data.totals || { total_quantity: 0, total_amount: 0 },
      );
      setTodayTotals(
        todayRes.data.totals || { total_quantity: 0, total_amount: 0 },
      );
  ...
  return (
    <Box className="h-full overflow-auto pr-1">
      <Box className="mb-5">
        ...
        <Typography variant="body2" className="mt-1 text-slate-500">
          Salom, {user?.first_name || "Ishchi"}. Bu oy bajargan ishlaringiz va
          hisobingiz.
        </Typography>
      </Box>
...
const AdminDashboard = ({ user }) => {
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    try {
      ...
      setClientStats({
        totalAmount:
          clientBalanceRes.data.balance?.total_amount ||
          salesRes.data.totals?.total_amount ||
          0,
        paidAmount:
          clientBalanceRes.data.balance?.paid_amount ||
          salesRes.data.totals?.paid_amount ||
          0,
        debtAmount:
          clientBalanceRes.data.balance?.debt_amount ||
          salesRes.data.totals?.debt_amount ||
          0,
  ...
  <Box className="h-full overflow-auto pr-1">
    ...
    >
      ...
      <Box className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <StatCard
          label="Aloqa"
          value="+998 91 571 70 09"
          helper="Tizim administratori"
        />
      </Box>
    </Paper>
  </Box>
