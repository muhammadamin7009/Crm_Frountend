...
const STAFF_ROLES = ["client", "supplier", "customer", "worker"];
...
const Users = () => {
  ...
  const fetchUsers = async (offset = 0, limit = pageInfo.limit) => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || "Hodimlarni olishda xato.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || "Hodim qo'shishda xato.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Hodimni yangilashda xato.",
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Hodimni o'chirishda xato.",
      );
    } finally {
      setDeleting(false);
    }
  ...
  const handleChangeRowsPerPage = (event) => {
    fetchUsers(0, Number(event.target.value));
  };
  ...
  return (
    <Box className="flex h-full min-h-0 flex-col">
        ...
        <Box className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <Box className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <TextField
              ...
              onChange={(e) => {
                ...
                fetchUsers(0, pageInfo.limit, e.target.value, sortOrder);
              }}
            >
          ...
          {canCreateUser && (
            <Button
              ...
              sx={{ borderRadius: 2, minHeight: 40, px: 3 }}
            >
              Hodim qo'shish
            </Button>
          )}
              ...
              {loading ? (
                ...
                users.map((user) => (
                  <TableRow
                    key={user.id}
                    hover
                    className={canOpenUserDetail ? "cursor-pointer" : ""}
                    onClick={() => {
                      if (canOpenUserDetail) navigate(`/users/${user.id}`);
                    }}
                    sx={{
                      "&:last-child td": { borderBottom: 0 },
                      "&:hover": canOpenUserDetail
                  ...
                  >
                    ...
                    <TableCell>
                      <Chip
                        ...
                        variant={
                          user.role === "customer" ? "outlined" : "filled"
                        }
                      />
                    </TableCell>
      ...
      <Dialog
        ...
        maxWidth="sm"
      >
        <DialogTitle sx={{ fontWeight: 800 }}>Hodim qo'shish</DialogTitle>
      ...
      <Dialog open={editOpen} onClose={closeEditModal} fullWidth maxWidth="sm">
        <DialogTitle sx={{ fontWeight: 800 }}>Hodimni tahrirlash</DialogTitle>
      ...
      <Dialog
        ...
        maxWidth="xs"
      >
        <DialogTitle sx={{ fontWeight: 800 }}>Hodimni o'chirish</DialogTitle>
