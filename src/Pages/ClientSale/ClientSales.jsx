...
const ClientSales = () => {
  ...
  const fetchSales = useCallback(
    async (offset = 0, limit = pageInfo.limit) => {
      ...
      try {
        ...
        setTotals(
          data.totals || { total_amount: 0, paid_amount: 0, debt_amount: 0 },
        );
        setPageInfo(data.pageInfo || { total: 0, offset, limit });
      } catch (error) {
        toast.error(
          error?.response?.data?.message || "Savdolarni olishda xato.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const fetchSummary = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Savdo summary olishda xato.",
      );
    } finally {
  ...
  const openCreateModal = () => {
    const product = products.find(
      (item) => Number(item.id) === Number(filters.product_id),
    );
    ...
    setPaymentBalance(
      sale
        ? {
            ...
            paid_amount: Number(
              sale.current_paid_amount || sale.paid_amount || 0,
            ),
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Savdoni o'chirishda xato.",
      );
    } finally {
      setDeleting(false);
    }
  ...
  const handleSavePayment = async () => {
    ...
    try {
      await createClientPayment({
        ...
        client_sale_id: paymentForm.client_sale_id
          ? Number(paymentForm.client_sale_id)
          : null,
    ...
    <Box className="flex h-full min-h-0 flex-col">
      ...
      >
        <Box className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
          <Box className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-9">
            <TextField
              size="small"
              label="Qidirish"
              value={filters.q}
      ...
      >
          ...
          <Table sx={{ minWidth: 1180 }}>
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={{ fontWeight: 700 }}>To'langan</TableCell>
                <TableCell sx={{ fontWeight: 700 }}>Qarz</TableCell>
              ...
              {loading ? (
                ...
                sales.map((sale) => (
                  <TableRow key={sale.id} hover>
                    ...
                    <TableCell>{formatMoney(sale.unit_price)}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Typography fontWeight={700}>
                        {formatMoney(
                          sale.current_paid_amount ?? sale.paid_amount,
                        )}
                      </Typography>
                    ...
                    <TableCell>
                      <Chip
                        size="small"
                        label={formatMoney(sale.debt_amount)}
                        color={
                          Number(sale.debt_amount) > 0 ? "warning" : "success"
                        }
                      />
                    </TableCell>
        ...
        <TablePagination
          ...
          onPageChange={(_, nextPage) =>
            fetchSales(nextPage * pageInfo.limit, pageInfo.limit)
          }
          onRowsPerPageChange={(event) =>
            fetchSales(0, Number(event.target.value))
          }
      ...
      <Dialog open={deleteOpen} onClose={closeModals} fullWidth maxWidth="xs">
        ...
        <DialogContent>
          <Typography>
            {selectedSale?.client_name} uchun{" "}
            {formatMoney(selectedSale?.total_amount)} savdo yozuvini
            o'chirmoqchimisiz?
          </Typography>
        </DialogContent>
        <DialogActions>
          ...
          <Stack spacing={2} className="pt-2">
            ...
            <Box className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <Box className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                ...
                <StatBox
                  label="To'langan"
                  value={formatMoney(paymentBalance.paid_amount)}
                />
                <StatBox
                  label="Qolgan qarz"
            ...
            <Box className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <TextField
                ...
                error={
                  ...
                  Number(paymentBalance.debt_amount || 0)
                }
                helperText={
                  Number(paymentForm.amount || 0) >
                  Number(paymentBalance.debt_amount || 0)
