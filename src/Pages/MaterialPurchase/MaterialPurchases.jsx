...
const MaterialPurchases = () => {
  ...
  const saveMaterial = async () => {
    if (!materialForm.name.trim())
      return toast.error("Homashyo nomini kiriting.");
    setSaving(true);
    try {
    ...
    <Box className="flex h-full min-h-0 flex-col">
      ...
      >
        <Box className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <Box className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <Button
              variant="outlined"
              onClick={() => fetchPurchases(0, pageInfo.limit)}
            >
              Filterlash
            </Button>
          </Box>
      ...
      >
          ...
          <Table sx={{ minWidth: 1050 }}>
            <TableHead>
              <TableRow>
                ...
                <TableCell>Qarz qo'shildi</TableCell>
              ...
              {loading ? (
                ...
                purchases.map((purchase) => (
                  <TableRow key={purchase.id} hover>
                    ...
                    <TableCell>{money(purchase.paid_amount)}</TableCell>
                    <TableCell>{money(purchase.debt_amount)}</TableCell>
                    <TableCell>{date(purchase.purchased_at)}</TableCell>
        ...
        <TablePagination
          ...
          onPageChange={(_, next) =>
            fetchPurchases(next * pageInfo.limit, pageInfo.limit)
          }
          onRowsPerPageChange={(e) => fetchPurchases(0, Number(e.target.value))}
